...
def main():
    ...
    for key, values in state_dict.items():
        if "_A" in key:
            B_init = values.clone().T
            state_dict[key]= torch.zeros_like(values)
        if "_B" in key:
            state_dict[key] = B_init
    ...
    class DPLoRA_Client(fl.client.NumPyClient):
        ...
        def get_projection_basis(self):
            ...
            if args.projection_type == "fixed":
                ...
                for k, v in state_dict.items():
                    if "lora_A" in k:
                        dA = prev_state_dict[k].cpu() - v.cpu()
                        print(k)
                        print(v)
                    elif "lora_B" in k:
                        B_v = v
                        ...
                        for i in range(dA.shape[0]):
                            if "1" in args.projection_type:
                                dBA_mag.append(torch.linalg.matrix_norm(B_v[:,i].unsqueeze(dim=1).cpu() @ dA[i,:].unsqueeze(dim=0)), ord=1)
                            if "2" in args.projection_type:
                                dBA_mag.append(torch.linalg.matrix_norm(B_v[:,i].unsqueeze(dim=1).cpu() @ dA[i,:].unsqueeze(dim=0)), ord=2)
                            else:
                                dBA_mag.append(torch.linalg.matrix_norm(B_v[:,i].unsqueeze(dim=1).cpu() @ dA[i,:].unsqueeze(dim=0)))
        ...
        def fit(self, parameters, config):
            self.set_parameters(parameters)
            if args.projection_type in ("gradient") or args.projection_type in ("BA_mag"):
