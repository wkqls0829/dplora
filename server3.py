...
init_ndarrays = [value.cpu().numpy() for value in state_dict.values()]

for i, k in enumerate(param_keys):
    print(k)
    print(init_ndarrays[i])
    ...
    def aggregate_fit(self, rnd, results, failures):
        ...
        for i, key in enumerate(param_keys):
            if "lora_A" in key: 
                ...
                aggregated_params.append(E_dist[:args.lora_r, np.newaxis] * A_dist[:args.lora_r, :])
                aggregated_params.append(B_dist[:, :args.lora_r])
