...
def main():
    ...
    class DPLoRA_Client(fl.client.NumPyClient):
        ...
        def fit(self, parameters, config):
            ...
            local_trainer=build_local_trainer(net=self.net ,
                                              local_train_dataset=train_data,
                                              local_eval_dataset=eval_data,
                                              optim="adamw_torch",
                                              tokenizer=tokenizer,
        ...
        def evaluate(self, parameters, config):
            self.set_parameters(parameters)
            eval_results = test(self.net, tokenizer, test_data, 1, args.micro_batch_size)
            print(eval_results)
            loss = eval_results["eval_loss"]
